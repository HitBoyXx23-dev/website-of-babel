...
function v12ArcadePage(){
  ...
  let cleanup=null;const stage=document.querySelector('#v12GameStage');document.querySelectorAll('[data-game]').forEach(b=>b.onclick=()=>{cleanup?.();cleanup=null;const g=b.dataset.game;if(g==='snake'||g==='pong'){stage.innerHTML=`<div class="v12-game-head"><b>${g==='snake'?'Snake':'Pong'}</b><span>${g==='snake'?'Arrow keys':'W/S or arrow keys'}</span></div><canvas width="${g==='snake'?400:480}" height="400" class="v12-game-canvas"></canvas>`;cleanup=g==='snake'?v12Snake(stage.querySelector('canvas')):v12Pong(stage.querySelector('canvas'));return}if(g==='mines'){stage.innerHTML='<div class="v12-game-head"><b>Mines</b><span>Click cells to reveal them.</span></div><div id="v12MinesHost"></div>';v12Mines(document.querySelector('#v12MinesHost'));return}if(g==='freedoom'){stage.innerHTML=`<div class="v12-game-head"><b>Doom engine / Freedoom</b><span>Open-source Doom-compatible game data. If embedding is blocked, use the open button.</span></div><iframe class="v12-media-frame" src="https://themagicalkarp.github.io/wasmdoom/" allow="fullscreen; gamepad; autoplay" referrerpolicy="no-referrer"></iframe><a class="v12-open-fallback" href="https://themagicalkarp.github.io/wasmdoom/" target="_blank" rel="noopener">Open Freedoom player</a>`;return}if(g==='badapple'){const fallback='https://archive.org/embed/bad-apple-resources';const show=(hit,note='Internet Archive fallback')=>{stage.innerHTML=`<div class="v12-game-head"><b>Bad Apple!!</b><span>${esc(hit?.source||note)}</span></div><iframe class="v12-media-frame v12-video-frame" src="${esc(hit?.embed||fallback)}" title="Bad Apple archive video" allow="fullscreen; autoplay" allowfullscreen referrerpolicy="no-referrer"></iframe><a class="v12-open-fallback" href="/archive?id=bad-apple-resources" data-route>Open archive record in Babel</a>`;wireRoutes()};stage.innerHTML=`<div class="v12-game-head"><b>Bad Apple!!</b><span>Searching non-YouTube public media indexes...</span></div><div class="v12-game-loading">Searching Internet Archive and public media catalogs...</div>`;json('/api/media-search?q='+encodeURIComponent('Bad Apple shadow animation')).then(d=>{const hit=(d.items||[]).find(x=>x.type==='video'&&x.embed);show(hit)}).catch(()=>show(null,'Internet Archive fallback · live search unavailable'))}});wireRoutes();
}
