...
function badAppleFallback(){
  return {
    type:'video',
    title:'Bad Apple!! shadow animation resources',
    url:'https://archive.org/download/bad-apple-resources/__ia_thumb.jpg',
    thumbnail:'https://archive.org/download/bad-apple-resources/__ia_thumb.jpg',
    embed:'https://archive.org/embed/bad-apple-resources',
    sourceUrl:'https://archive.org/details/bad-apple-resources',
    identifier:'bad-apple-resources',
    source:'Internet Archive',
    creator:'community archive',
    description:'Non-YouTube Internet Archive fallback for Bad Apple media playback and research.'
  };
}
...
module.exports=async(req,res)=>{
  ...
  if(/\bbad\s*apple\b/i.test(q)){
    const item=badAppleFallback();
    return send(res,200,{query:q,count:1,counts:{video:1},providers:['Internet Archive'],items:[item],note:'Babel uses a known non-YouTube Internet Archive fallback for Bad Apple so playback does not depend on a live federated search.'});
  }
