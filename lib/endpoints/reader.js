...
function extractMedia(html,base){
  const images=[];const videos=[];let m;
  const imgRe=/<img\b[^>]*>/gi;while((m=imgRe.exec(html))&&images.length<30){const tag=m[0],src=absolute(attr(tag,'src')||attr(tag,'data-src'),base);if(!src)continue;const alt=clean(attr(tag,'alt')).slice(0,180);if(/sprite|logo|icon|avatar/i.test(src)&&!alt)continue;images.push({url:src,alt})}
  const og=absolute(meta(html,'og:image'),base);if(og)images.unshift({url:og,alt:''});
  const vidRe=/<(?:video|source|iframe)\b[^>]*>/gi;while((m=vidRe.exec(html))&&videos.length<20){const tag=m[0],src=absolute(attr(tag,'src'),base);if(!src)continue;if(/youtube\.com|youtu\.be|vimeo\.com|archive\.org|\.mp4(?:\?|$)|\.webm(?:\?|$)|\.ogg(?:\?|$)/i.test(src))videos.push({url:src})}
  return {images:[...new Map(images.map(x=>[x.url,x])).values()].slice(0,20),videos:[...new Map(videos.map(x=>[x.url,x])).values()].slice(0,12)};
}
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    return send(res,200,{url:page.url,host:u.hostname.replace(/^www\./,''),resource:false,contentType:page.type,size:page.length,title,description,sections:extractSections(html),images:media.images,videos:media.videos,links,generatedAt:new Date().toISOString(),note:'Babel Reader is an on-demand normalized preview. It does not claim to mirror or permanently store the source page.'});
