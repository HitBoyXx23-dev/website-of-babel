...
const anchors=[
  {terms:['hitboyxx23','hitboyxx23-dev','hitboy','hitboyxx23.dev'],title:'HitBoyXx23',url:'https://hitboyxx23.dev/',snippet:'HitBoyXx23 public website. Babel crawls the site root to discover projects, stack, contact, music, anime, blog posts, and other linked pages.',source:'Verified web anchor',kind:'profile'},
...
async function siteDiscovery(q){
  const start=queryToURL(q);
  if(!start)return [];
  try{
    const crawl=await crawlSite(start,{depth:2,maxPages:32,maxNodes:150,maxEdges:220,includeSitemap:true});
    return (crawl.pages||[]).map((x,i)=>({
      title:x.title||x.path||x.host,
      url:x.url,
      snippet:x.description||`Page on ${x.host}${x.path||''}`,
      source:i===0?'Direct site':'Site crawl',
      kind:'website',
      siteDepth:x.depth||0,
      path:x.path||'/'
    }));
...
module.exports=async(req,res)=>{
  ...
  const [site,br,ddg,wiki,gh,gl,so,exact]=await Promise.all([siteDiscovery(q),brave(q),duck(q),wikipedia(q),githubUsers(q),gitlabUsers(q),stackUsers(q),exactHandleChecks(q)]);
  const results=uniq([...pinned,...site,...exact,...br,...ddg,...gh,...gl,...so,...wiki]).slice(0,120);
  const providers={anchors:pinned.length,siteCrawl:site.length,exactHandles:exact.length,brave:br.length,duckduckgo:ddg.length,github:gh.length,gitlab:gl.length,stackoverflow:so.length,wikipedia:wiki.length};
  return send(res,200,{query:q,results,count:results.length,providers,live:ddg.length>0||br.length>0||site.length>0,note:'Results combine direct site crawling for domain queries, public web search, reference search, developer-network search, and exact handle checks. Matching handles across services are not proof that the accounts belong to the same person.'});
};
