...
function uniq(rows){const seen=new Set();return rows.filter(r=>{if(!r?.url||!/^https?:\/\//i.test(r.url))return false;const k=r.url.replace(/\/$/,'');if(seen.has(k))return false;seen.add(k);r.host=r.host||host(r.url);return true})}
...
async function brave(q){
  ...
  try{const d=await fetchJSON(`https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(q)}&count=20`,{headers:{'Accept':'application/json','X-Subscription-Token':key}});return (d.web?.results||[]).map(x=>({title:x.title||x.url,url:x.url,snippet:clean(x.description||''),source:'Brave Search',kind:'website'}))}catch{return []}
}
