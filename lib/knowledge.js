...
const seed=[...require('../data/seed'),...require('../data/extra-seed'),...require('../data/domain-seed'),...require('../data/esoteric-cultivation-seed'),...require('../data/influence-seed')];
...
function classify(q){
  ...
  if(/manipulat|persuasion|rhetoric|negotiat|propaganda|misinformation|disinformation|gaslight|coercive|social engineering|phishing|dark pattern|influence|brainwash|subliminal|grooming|scam|groupthink|conformity|obedience/.test(x))return ['Manipulation & Influence','Psychology / communication; evidence and ethics vary by technique'];
  ...
  if(/farm|agricultur|cultivat.*crop|crop|soil|irrigat|hydropon|greenhouse|horticultur|permacultur|compost/.test(x))return ['Agriculture & Cultivation','Established agricultural / practical knowledge'];
  if(/qi cultiv|qigong|neigong|neidan|dantian|jing qi shen|daoist cultiv|xianxia|xuanhuan|wuxia|internal alchemy/.test(x))return ['Qi Cultivation','Traditional / historical; physiological effects vary by practice'];
  if(/witch|wicca|pagan|grimoire|ceremonial magic|ritual magic|sigil|talisman|occult|alchemy|esoteric|astral|chakra|demonolog|angelolog|goetia|theurgy|hermetic/.test(x))return ['Occult & Esoteric','Historical / religious / folkloric; supernatural claims vary'];
